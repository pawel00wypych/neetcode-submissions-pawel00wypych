class Solution:
    def maxDistance(self, arrays: List[List[int]]) -> int:
        ...
        for m in range(2, len(arrays)):
            if m not in visited_indexes and arrays[m][0] < global_min and max_diff < abs(global_max-arrays[m][0]):
                global_min = arrays[m][0]
                visited_indexes.add(m)
                max_diff = abs(global_max - global_min)
            elif m not in visited_indexes and arrays[m][-1] > global_max and max_diff < abs(arrays[m][-1] - global_min):
